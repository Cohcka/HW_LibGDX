...
public class GameController {
    ...
    public boolean checkObjectByCO(Vector2 pos, int bulletSize, int damage){
        for (int i = 0; i < tanksController.activeSize(); i++) {
            if(tanksController.getActiveList().get(i).getOwnerType() == Tank.Owner.valueOf("AI")){
                tmp.set(tanksController.getActiveList().get(i).getPosition());
                if(tmp.dst(pos) < bulletSize + 30){
                    tanksController.dealingDamage(tanksController.getActiveList().get(i), damage);
                    return true;
                }
            }
        }
